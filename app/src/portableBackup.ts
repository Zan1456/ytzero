...
export const BACKUP_SECTIONS: readonly BackupSectionDefinition[] = [
  ...
  { id: "profile.settings", schemaVersion: 6, scope: "profile", sensitivity: "normal", dependencies: ["profiles.index"], category: "configuration", path: profilePath("settings.json") },
  ...
  try {
    ...
    const tx = database.transaction(async () => {
      ...
      for (const profile of state.manifest.profiles) {
        ...
        if (selected.has("profile.bookmarks")) { if (state.plan!.strategy === "replace") await database.prepare("DELETE FROM bookmarks WHERE user_id=?").run(uid); for (const row of get("profile.bookmarks")??[]) { const position=Number(row.position_seconds); if (typeof row.video_id!=="string" || !Number.isFinite(position) || position<0 || !await database.prepare("SELECT 1 FROM videos WHERE video_id=?").get(row.video_id)) { counts.skipped++; continue; } const uuid=typeof row.uuid==="string"&&UUID.test(row.uuid)?row.uuid:crypto.randomUUID(); await database.prepare("INSERT INTO bookmarks(portable_uuid,user_id,video_id,position_seconds,description,created_at,updated_at) VALUES(?,?,?,?,?,?,?) ON CONFLICT(user_id,video_id) DO UPDATE SET position_seconds=excluded.position_seconds,description=excluded.description,updated_at=excluded.updated_at").run(uuid,uid,row.video_id,Math.min(position,1_000_000_000),String(row.description??"").slice(0,2000),row.created_at??new Date().toISOString(),row.updated_at??row.created_at??new Date().toISOString()); } }
